...
type Node = { x: number; y: number; tone: Tone; size: number; delay: number; dot?: boolean; link?: boolean; tag?: string };

// positions are % of the square stage; center scene sits around 50/50
const NODES: Node[] = [
  { x: 13, y: 19, tone: 'violet', size: 15, delay: 0, dot: true, link: true },
  { x: 87, y: 13, tone: 'violet', size: 12, delay: 1.1, link: true, tag: 'quietly online' },
        ...
        >
          ...
          >
            ...
            >
              ...
              {n.tag && (
                <div className="absolute bottom-[76%] left-[72%] flex items-center gap-1.5 whitespace-nowrap rounded-full border border-lw-line bg-lw-ink/80 px-2.5 py-1 font-lw-body text-[0.66rem] font-semibold text-lw-text-dim backdrop-blur-md shadow-[0_8px_20px_rgba(0,0,0,0.35)]">
                  <span
                    className="size-1.5 rounded-full motion-safe:animate-lw-blink"
                    style={{ background: tone.glyph, boxShadow: `0 0 6px ${tone.glow}` }}
                  />
                  {n.tag}
                </div>
